...
def sort_by_number_of_routes(area: list):
    def routes(item: ClimbingArea):
        return item.routes()
    result = sorted(area, key=routes)
    return result


def sort_by_most_difficult(area: list):
    def difficult(item: ClimbingArea):
        return item.hardest_route().grade
    result = sorted(area, key=difficult, reverse=True)
    return result
