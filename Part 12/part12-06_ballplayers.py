...
def most_goals(items: list):
    return max(items, key=lambda x: x.goals).name


def most_points(items: list):
    return max(items, key=lambda x: x.goals + x.passes).name, max(
        items, key=lambda x: x.goals + x.passes).number


def least_minutes(items: list):
    return min(items, key=lambda x: x.minutes)
